...
begin
  ...
  repeat
    ...
    begin
      ...
      if nID[1] = 0 then
      begin
        MessageDlg('Unknown OBJ format, contact racingculturegr@gmail.com', mtError, [mbOK], 0);
        CloseFile(f);
        Exit;
      end;
...
function ReadLWOIndex(var f: file; var c: array of AnsiChar): Integer;
var
  b1,b2,b3,b4: Integer;
begin
  BlockRead(f,c,2);
...
var
  ...
  PTag: array [1..65536] of Byte; //1=surf 2=part 3=color
...
repeat
  ...
  if chsize<>0 then
           ...
           //   ShowMessage('Chunk: ' + chname + ' Size: ' + IntToStr(chsize));
...
else if chname='VMPA' then
begin
  BlockRead(f,c,chsize);
end
else if chname='VMAP' then
begin
  BlockRead(f,c,6);
  Dec(chsize,6);
...
begin
  ...
  begin
    ...
    if (k<>3) and (k<>4) then
    begin
      { Skip non-triangle/non-quad polygon }
      for i:=1 to k do
      begin
        vx:=ReadLWOIndex(f,c);
        Dec(chsize,2);
    ...
    begin
      ...
      vx:=ReadLWOIndex(f,c);
      Dec(chsize,2);
      Imp.Faces[Imp.PolyCount+LWOQty.Poly[lay],1]:=
        vx+Imp.VerticeCount-LWOQty.XYZ[lay]+1;

      vx:=ReadLWOIndex(f,c);
      Dec(chsize,2);
      Imp.Faces[Imp.PolyCount+LWOQty.Poly[lay],2]:=
        vx+Imp.VerticeCount-LWOQty.XYZ[lay]+1;

      vx:=ReadLWOIndex(f,c);
      Dec(chsize,2);
      Imp.Faces[Imp.PolyCount+LWOQty.Poly[lay],3]:=
        vx+Imp.VerticeCount-LWOQty.XYZ[lay]+1;
      ...
      begin
        vx:=ReadLWOIndex(f,c);
        Dec(chsize,2);
        Imp.Faces[Imp.PolyCount+LWOQty.Poly[lay],4]:=
          vx+Imp.VerticeCount-LWOQty.XYZ[lay]+1;
      end
      else
        Imp.Faces[Imp.PolyCount+LWOQty.Poly[lay],4]:=0;
    end;
  end;
  ...
  begin
      ...
      begin
        ...
        for i:=1 to (chsize div 4) do begin
          ...
          j := ord(c[3])*256 + ord(c[4]) + 1;

if j > High(PTag) then
begin
  MessageDlg(
    Format('LWO PART PTAG index too large: %d (max %d)',
      [j, High(PTag)]),
    mtError, [mbOK], 0
  );
  CloseFile(f);
...
if j <= High(PTag) then
begin
  Imp.Surf[k+Imp.PolyCount-LWOQty.Poly[lay]] := j;
  Ptag[j] := 1;
end
else
begin
  Imp.Surf[k+Imp.PolyCount-LWOQty.Poly[lay]] := 0;
end;
...
begin
  ...
  begin
    MessageDlg(
      'Uknown error happened while processing LWO file',
      mtError, [mbOK], 0
    );
    CloseFile(f);
    Exit;
...
begin
  while chsize > 0 do
  begin
    if chsize > MAX_READ_BUFFER then
      j:=MAX_READ_BUFFER
    else
      j:=chsize;
    if FilePos(f)+j > FileSize(f) then
...
until(Ptag[k]=0); //now CW stands for number of colorwires
  ...
  ShowMessage(
  'XYZ=' + IntToStr(Imp.VerticeCount) +
  ' POLY=' + IntToStr(Imp.PolyCount) +
  ' SURF=' + IntToStr(MatCount)
);
